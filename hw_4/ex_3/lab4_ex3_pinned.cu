...
int main(int argc, char **argv) {
    ...
    cudaHostAlloc(&hostA, numARows * numAColumns * sizeof(DataType), cudaHostAllocDefault); // hostA[i*numAColumns + j] is equivalent to the usual hostA[i][j]
    cudaHostAlloc(&hostB, numBRows * numBColumns * sizeof(DataType), cudaHostAllocDefault); // hostB[i*numBColumns + j] is equivalent to the usual hostB[i][j]
    cudaHostAlloc(&hostC, numCRows * numCColumns * sizeof(DataType), cudaHostAllocDefault); // hostC[i*numCColumns + j] is equivalent to the usual hostC[i][j]
    ...
    //@@ Free the (pinned) CPU memory here
    cudaFree(hostA);
    cudaFree(hostB);
    cudaFree(hostC);
    cudaFree(resultRef);
